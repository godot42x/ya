...
do
    ...
    add_configs("docking", { description = "Enable ImGui Docking branch", default = false, type = "boolean" })

    on_load(function(package)

        local is_docking = package:config("docking")
        local pkg = is_docking and "imgui docking" or "imgui"
        if package:gitref() or package:version():ge("1.90") then
            package:add("deps", pkg)
        else
            package:add("deps", pkg .. " <=1.89.3")
        end
    end)
