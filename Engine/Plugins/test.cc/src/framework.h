...
#if defined(_WIN32) and defined(BUILD_SHARED)
    #if defined(TEST_CC_BUILD_SHARED)
        #define TEST_CC_API __declspec(dllimport)
    #else
        #define TEST_CC_API __declspec(dllexport)
    #endif
#else
    #define TEST_CC_API
#endif
...
struct TestCaseRegistrar
{
    ...
    {
        TestRegistry::RegisterTest(name, []() -> bool { return TestClass::Run(); }, file, line);
    }
};
} // namespace TestFramework
