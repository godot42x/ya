...
function detectBuildSystem() {
    ...
    const buildSystem = config.get('buildSystem', 'auto');
    if (buildSystem !== 'auto') {
        return buildSystem;
    }
    // Auto-detect
...
function getTestRunnerPath() {
    const config = vscode.workspace.getConfiguration('ya.test');
    const customPath = config.get('testRunnerPath');
    if (customPath && customPath !== '${ya.test.buildDir}/test_runner.exe') {
        // Replace variables
        const projectRoot = getProjectRoot();
        return customPath
            .replace('${workspaceFolder}', projectRoot)
            .replace('${ya.test.buildMode}', config.get('buildMode', 'debug'));
    }
...
function updateDiagnostics(document, testName, result) {
    ...
    if (match && !result.passed) {
        ...
        const diagnostic = new vscode.Diagnostic(range, result.errorMessage || 'Test failed', vscode.DiagnosticSeverity.Error);
...
function activate(context) {
    ...
    try {
        const testRunnerPath = getTestRunnerPath();
        const projectRoot = getProjectRoot();
        testController = new testController_1.TestController(context, testRunnerPath, projectRoot);
    }
    catch (error) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('ya.debugTest', async (testName) => {
        const testRunnerPath = getTestRunnerPath();
        // Check if test runner exists
        if (!fs.existsSync(testRunnerPath)) {
            const buildSuccess = await buildTests();
