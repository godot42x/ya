...
//     #define ENABLE_DIRECTIONAL_SHADOW 0
// #endif
...
// #endif
    ...
    vec2 texSize = 1.0 / textureSize(uDirectionalLightShadowMap, 0); 
    for (int x = -1; x <= 1; ++x) {
        for (int y = -1; y <= 1; ++y) {
            vec2 offset = vec2(x, y) * texSize;
            float pcfDepth = texture(uDirectionalLightShadowMap, uv + offset).r; // get depth from shadow map
            shadow += (curDepth - bias) > pcfDepth ? 1.0 : 0.0; // compare depth with bias
        }
    }
    shadow /= 9.0; // average the shadow factor
#else
    float closestDepth = texture(uDirectionalLightShadowMap, uv).r; // determined by the sampler is nearest or linear
