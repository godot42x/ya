...
def generate_source_only_header(output_path: str, namespace: str, slang_source: str) -> bool:
    """Emit a header containing constants/enums without requiring reflection JSON."""
    extract_defines(slang_source)
    extract_enums(slang_source)

    if not SLANG_DEFINES and not SLANG_ENUMS:
        return False
...
def _collect_dependency_source(slang_file: Path, include_dirs: list[Path] | None) -> str:
    """Return concatenated source text for the slang file and all local includes.

    This allows header generation to resolve constants/enums declared in shared
    include files such as Common/Limits.slang instead of relying on external
    config inputs.
    """
    chunks: list[str] = []
    for dependency in _iter_slang_dependencies(slang_file, include_dirs):
        chunks.append(dependency.read_text(encoding="utf-8"))
    return "\n".join(chunks)


def _has_generatable_source_members(slang_source: str) -> bool:
    stripped = _strip_comments(slang_source)
    return (
        re.search(r'^\s*#\s*define\s+[A-Z_][A-Z0-9_]*\s+', stripped, re.MULTILINE) is not None
        or re.search(r'\benum\s+[A-Za-z_][A-Za-z0-9_]*\b', stripped) is not None
    )
...
def _process_one_slang(
    ...
    dependency_source = _collect_dependency_source(slang_file, include_dirs)
    ...
    if entry_stage is None:
        if _has_generatable_source_members(slang_source):
            output_dir.mkdir(parents=True, exist_ok=True)
            if generate_source_only_header(str(header_path), full_ns, slang_source):
                print(f"[slang-gen] Generated source-only header: {header_path}")
                return
    ...
    generate_header(str(json_path), str(header_path), full_ns, dependency_source)
