...
class RenderContext
{
  public:
     ...
     */
    IRenderTarget *createSwapchainRenderTarget(IRenderPass *renderPass);
     ...
     * @return Pointer to the created RenderTarget (managed by caller)
     */
    IRenderTarget *createRenderTarget(IRenderPass *renderPass, uint32_t width, uint32_t height, uint32_t bufferCount = 1);

    /**
     * @brief Register a RenderTarget with this context
     * Used when RenderTarget is created externally
     */
    void registerRenderTarget(IRenderTarget *target);

    /**
     * @brief Unregister and optionally delete a RenderTarget
     */
    void destroyRenderTarget(IRenderTarget *target);
