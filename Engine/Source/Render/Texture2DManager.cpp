...
std::shared_ptr<Texture2DManager::Texture2D> Texture2DManager::createVulkanTexture(const void* data, uint32_t width, uint32_t height, uint32_t channels) {
    ...
    try {
        // Create image
        VulkanUtils::createImage(device, physicalDevice, width, height, format,
        ...
        if (data) {
            ...
            VulkanUtils::transitionImageLayout(device, commandPool, graphicsQueue,
                                             texture->image, format,
                                             VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
            
            VulkanUtils::copyBufferToImage(device, commandPool, graphicsQueue,
