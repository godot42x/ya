...
private:
    ...
    std::unordered_map<T_INTEGER, std::unique_ptr<PipelineInfo>> m_pipelines;
...
public:
    ...
    bool createPipeline(const FName &name, const GraphicsPipelineCreateInfo &config, 
    ...
    bool createPipeline(const std::string &name, const GraphicsPipelineCreateInfo &config, 
                       VkRenderPass renderPass, VkExtent2D extent) {
        return createPipeline(FName(name), config, renderPass, extent);
    }
