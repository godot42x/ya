...
void DeferredPhongRenderPath::init(DeferredRenderPipeline& pipeline)
{
    ...
    YA_CORE_ASSERT(_gBufferPipeline && _gBufferPipeline->recreate(gBufferPipelineCI), "Failed to create deferred phong GBuffer pipeline");
    ...
    YA_CORE_ASSERT(_lightPipeline && _lightPipeline->recreate(lightPipelineCI), "Failed to create deferred phong light pipeline");

    _deferredDSP = IDescriptorPool::create(
        render,
        DescriptorPoolCreateInfo{
            .label     = "Deferred_Phong_DSP",
            .maxSets   = 2,
            .poolSizes = {
                DescriptorPoolSize{
    ...
    _frameUBO = IBuffer::create(
        render,
        BufferCreateInfo{
            .label         = "Deferred_Phong_Frame_UBO",
    ...
    _lightUBO = IBuffer::create(
        render,
        BufferCreateInfo{
            .label         = "Deferred_Phong_Light_UBO",
            .usage         = EBufferUsage::UniformBuffer,
...
void DeferredPhongRenderPath::shutdown(DeferredRenderPipeline& pipeline)
{
    (void)pipeline;
    _lightUBO.reset();
    _frameUBO.reset();
...
void DeferredPhongRenderPath::beginFrame(DeferredRenderPipeline& pipeline)
{
    (void)pipeline;
...
void DeferredPhongRenderPath::tick(DeferredRenderPipeline& pipeline, const DeferredRenderTickDesc& desc, Scene* scene)
{
    YA_CORE_ASSERT(scene, "DeferredPhongRenderPath requires a valid scene");
    ...
    desc.cmdBuf->bindPipeline(_gBufferPipeline.get());
    ...
    desc.cmdBuf->setViewport(0.0f, gBufferViewportY, static_cast<float>(viewportWidth), gBufferViewportHeight);
    desc.cmdBuf->setScissor(0, 0, viewportWidth, viewportHeight);
    ...
    for (const auto& [entity, mc, tc, pmc] : drawListView.each()) {
        ...
        uint32_t idx = static_cast<uint32_t>(material->getIndex());
        desc.cmdBuf->bindDescriptorSets(
            _gBufferPPL.get(),
            0,
            {
                _frameAndLightDS,
    ...
    desc.cmdBuf->beginRendering(lightPassRI);
    ...
    desc.cmdBuf->bindPipeline(_lightPipeline.get());
    desc.cmdBuf->setViewport(0.0f, 0.0f, static_cast<float>(viewportWidth), static_cast<float>(viewportHeight));
    desc.cmdBuf->setScissor(0, 0, viewportWidth, viewportHeight);
    desc.cmdBuf->bindDescriptorSets(
        _lightPPL.get(),
        0,
    ...
    quad->draw(desc.cmdBuf);

    if (pipeline._skyboxSystem && pipeline._skyboxSystem->bEnabled) {
        desc.cmdBuf->debugBeginLabel("Deferred Skybox");
...
void DeferredPhongRenderPath::renderGUI(DeferredRenderPipeline& pipeline)
{
    (void)pipeline;
    if (_gBufferPipeline) {
