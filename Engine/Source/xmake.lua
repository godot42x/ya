...
-- Forbidden include prefixes per module (config-time dependency lint).
-- Key: module name; value: list of engine include prefixes the module's own
-- sources (headers + cpp) must never reference. These mirror the boundary
-- rules in .agent/plan/module-boundary-cleanup/plan.md §6.
local YA_FORBIDDEN_INCLUDES = {
...
--- Build-time include lint (runs before every module build): every module's
--- own sources are scanned for forbidden engine include prefixes; violations
--- raise so a missing add_deps() or a wrong-direction edge fails loudly
--- instead of linking through tier roots (which no longer exist).
local function check_forbidden_includes(target)
    local forbidden = YA_FORBIDDEN_INCLUDES[target:name()]
    if not forbidden then
        return
    end
    for _, sourceFile in ipairs(target:sourcefiles()) do
        if sourceFile:find("/ThirdParty/", 1, true) then
            goto continue
        end
        local contents
        local absPath = path.absolute(sourceFile, os.projectdir())
        if io and io.readfile then
            contents = io.readfile(absPath)
        end
        if not contents then
            goto continue
        end
        for _, prefix in ipairs(forbidden) do
            if contents:find('#include%s*[<"]' .. prefix, 1) then
                raise("forbidden include: %s references \"%s\" (module %s)", sourceFile, prefix, target:name())
            end
        end
        ::continue::
    end
