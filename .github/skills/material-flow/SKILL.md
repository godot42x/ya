...
- 只负责已有组件的运行时资源解析
- 不创建 scene topology
- 当前职责包括：Mesh / PhongMaterial / PBRMaterial / UI / Billboard / Skybox resolve
...
5. **GPU UBO 结构体必须使用 ya-shader（`slang_gen_header.py`）从 `.slang` 生成的 C++ 类型（`slang_types::` 命名空间）作为唯一数据源。C++ 侧不得自行定义与 shader uniform 对应的结构体。** 示例：`PBRMaterial::ParamUBO` 应为 `using ParamUBO = slang_types::DeferredRender::PBR_GBufferPass::PBRParamsData;`，而非手写 `struct ParamUBO { ... };`。这保证 C++/GPU 内存布局始终一致，消除 std140 padding 不匹配风险。
