...
- 单个 `.cpp` / `.h` 文件尽量不要超过 1000 行；超过时优先按功能块、helper、pipeline 等稳定职责拆分，而不是继续堆在一个文件里
- 文件命名按类名或稳定职责收敛，例如 `AssetTextureManager.h`、`AssetModelManager.cpp`、`AssetTextureImport.cpp`；不要新增 `module.part.h` / `module.part.cpp` 这种分段命名
- 目录也按功能和层级收敛；例如 facade/owner 放到 `Resource/Manager/`，texture 专属 helper 与导入逻辑放到 `Resource/Texture/`，避免单个目录平铺过多无关类
