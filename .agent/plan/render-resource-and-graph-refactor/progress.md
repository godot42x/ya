...
- 2026-07-16：`shadow 开关和分辨率变化` 与 `SSAO 开关` 这两条 runtime 验证项现在也补成了和前几批一致的版本化 smoke。新增 `.agent/plan/render-resource-and-graph-refactor/shadow-smoke.automation.json` 后，当前工作树已实际跑通 `shadow.quality=high + shadow.directionalEnabled=false + shadow.resolution=3072` 这条组合路径，并产出 `/tmp/shadow-smoke-2026-07-16.png`（sha1 `5f5c72326d8027282481ef82b0e2d9aec828843d`）；截图里地面的方向光阴影已被移除，但局部光照和环境反射保持正常，说明 toggle 与 shadow-resource rebuild 边界在当前基线下可用。与此同时，新增 `.agent/plan/render-resource-and-graph-refactor/ssao-disabled-smoke.automation.json` 后，`smoke.deferred.ssao.enabled=false` 这条路径也已实际跑通，并产出 `/tmp/ssao-disabled-smoke-2026-07-16.png`（sha1 `80d72f99f32432782c4e6e14fae2a0f4f1980d2f`）；截图里接触阴影相对默认 Deferred 基线明显减弱，说明 SSAO override 已确实作用到运行时。两条运行对应的 `Engine/Saved/Logs/YA-2026-07-17_05-10-21.log` 中没有 `Validation Error`、`VUID-` 或 `[Error]`。
- 直接收益：`todo.md` 底部此前剩下的两个高价值 runtime toggle 项现在终于也有了固定 config、固定机位、固定抓图帧和当前工作树证据。后续如果继续动 shadow resource rebuild、Deferred light/SSAO 输入、postprocess 输出或 editor screenshot 观察面，不需要再临时去改本地 editor 配置或手搓一条一次性命令。
- 当前停止线：这一步仍然没有把更后的 `关键截图基线对比` 或 extension 相关运行项提前勾掉，也没有把 shadow 全链路实现层面的 owner/lifetime 问题宣称已完结。当前补齐的是“toggle/override 路径可复用、可回归”的 runtime smoke 证据；真正更高层的实现收口仍要继续围绕 runtime/shutdown/registry boundary 往下做。

- 2026-07-16：`screenshot/readback` 与 `shutdown 无 validation/lifetime error` 这两条 runtime 验证项现在也有了一条更贴近主计划关切边界的联合 smoke。新增 `.agent/plan/render-resource-and-graph-refactor/shutdown-readback-smoke.automation.json` 后，当前工作树已实际跑通 `HelloMaterial + editor screenshot target + frame 180 presentation readback + frame 220 graceful shutdown` 这条路径，并产出 `/tmp/shutdown-readback-smoke-info-2026-07-16.png`（sha1 `33ecc9cf14d4eb085dea24c5288a93b70d63fef3`）。同一次 `info/warn` 运行里，日志明确出现了 `Saved screenshot`、`Automation requested graceful shutdown after frame 220` 和 `Application exited successfully`，同时 `Engine/Saved/Logs/YA-2026-07-17_05-06-03.log` 中没有 `Validation Error`、`VUID-` 或 `[Error]`。这说明当前 graph-backed presentation readback、PNG 写出、readback buffer finalize、deferred deletion flush 和退出链在同一条自动化路径上已能稳定协同工作。
- 直接收益：底部持续审查项里此前还保守挂着的 `screenshot/readback` 与 `shutdown 无 validation/lifetime error` 现在终于有了比“短帧退出无崩溃”更强的当前态证据，而且这条 smoke 后续可以直接复用来盯 `presentation output`、imported final-state、readback buffer、shutdown 清理与 DDQ flush 的交界，不需要每次再手拼一条临时命令。
- 当前停止线：这一步没有顺手把 `shadow` 或 `SSAO` 的独立开关 smoke 一并宣称完成，也没有把更后的 `关键截图基线对比` 提前勾掉。当前落档的是“editor screenshot readback + graceful shutdown” 这条高价值联合路径；更细的视觉对比和 shadow/SSAO 单项开关仍需后续分别补证据。

- 2026-07-16：在退出链已经有 shutdown/readback 联合 smoke 证据的基础上，`RenderRuntime::shutdown()` 的 owner teardown 顺序又补实了一步。此前 runtime 会先 `shutdownRuntimeServices()`，其中包含 `ImGuiManager::shutdown()` 与 `ResourceRegistry::clearAll()`，然后才销毁 active pipeline；这让 graph executor、postprocess/shadow/viewport 等仍归 pipeline 持有的运行态对象，要和已经清掉的 runtime service / registry 交错退场。现在 shutdown 会先在需要时显式 `waitIdle()`，再先 `shutdownActivePipeline()` 与 debug render system，最后才走 `shutdownRuntimeServices(/*bRenderAlreadyIdle=*/true)`、`_deleter.clear()` 与 backend destroy。这样 pipeline/graph/stage owner 会先于 runtime service/cache 退场，退出链上的 owner 顺序终于更接近当前代码现实里的资源依赖方向，也与前面持续压实的 deferred deletion / registry replacement 边界更一致。
- 直接收益：这一步没有扩大成新的 shutdown 架构设计，但它确实把一个真实的 teardown 顺序缝隙收紧了：active pipeline 不再在 `ResourceRegistry::clearAll()` 之后才开始析构，后续如果继续看 shutdown validation、presentation/readback finalize、registry clear 与 provider/deleter 的交界，观察到的问题会更像真实 owner 关系，而不是被过早清 runtime service 的顺序噪音掩盖。
- 当前停止线：这次没有宣称 runtime shutdown 已经彻底摆脱全局 `waitIdle()`，也没有把 pipeline switch、scene/play-mode teardown 或更细的 submission retirement contract 一并解决；它只修正了 runtime 总退出链上“pipeline 比 runtime services 更晚退场”的当前不一致。

- 2026-07-16：`RenderRuntime::applyPendingRenderPipelineSwitch()` 这条 runtime 主边界也从“帧开始前整机 idle”收紧回了现有 frame-boundary contract。此前 render runtime 会在 `renderFrame()` 一进来就先切 pipeline，并在 switch 内直接 `_render->waitIdle()`；但代码复盘确认 Vulkan 当前 `flightFrameSize = 1`，而 `RenderRuntime::beginFrameCommandBuffer()` 调用的 `_render->begin()` 已经会在每帧起点等待上一帧 fence、flush `DeferredDeletionQueue` 并完成 swapchain acquire。现在 pipeline switch 改为发生在 `prepareFrame()` 成功之后，也就是已经经过这套标准帧边界同步之后；switch 自身不再额外做全局 idle，而是继续只负责旧 pipeline teardown、新 pipeline init 与 viewport 状态回放。
- 直接收益：这一步没有把 pipeline switch 宣称为“完全无同步成本”，但它确实去掉了一次落在主热路径最前面的保守全局等待。后续如果继续看 pipeline switch、registry replacement、presentation/imported wrapper 或 debug viewport 的交界，看到的行为会更接近现有 per-frame fence + DDQ contract，而不是先被 `waitIdle()` 把所有潜在时序差异抹平。
- 当前停止线：这次结论依赖的是当前 Vulkan `flightFrameSize = 1` 与单帧 fence wait 语义，没有顺手把 Forward/Deferred 各自内部的 shadow/resource rebuild `waitIdle()` 一并删除，也没有把将来多 frames-in-flight 下的 switch 语义提前假定为已经成立；它只收掉了当前 render runtime 主帧入口上一处已经被既有 frame-boundary contract 覆盖的全局等待。

- 2026-07-16：shutdown 主链里 `shutdownRuntimeServices()` 的内部 contract 也收得更明确了。此前 `RenderRuntime::shutdown()` 已经在最外层根据 `bRenderAlreadyIdle` 决定是否先做一次全局等待，并在 active pipeline teardown 之后调用 `shutdownRuntimeServices(/*bRenderAlreadyIdle=*/true)`；但 `shutdownRuntimeServices()` 自己仍保留一条“如果还没 idle 就再 wait 一次”的私有分支，尽管当前代码里已经没有任何调用面会走到它。现在这个 helper 改为无参，并显式断言 `_forwardPipeline/_deferredPipeline` 已经先退场，再执行 `ImGuiManager::shutdown()` 与 `ResourceRegistry::clearAll()`。这样 shutdown 的等待责任继续只留在最外层 `shutdown()`，runtime services 这层不再保留一个实际上不会被使用的第二套 idle 语义。
- 直接收益：这一步不是新的大行为改动，但它把前面已经收紧过的 owner teardown 顺序进一步固化成代码 contract。后续如果再审 shutdown/readback、registry clear 或 provider/deleter 交界，不需要再分心判断“runtime services 会不会自己再做一套等待”；等待责任和 teardown 顺序现在都集中在同一处。
- 当前停止线：这次没有改 `RenderRuntime::shutdown(bool)` 对外保留的 `bRenderAlreadyIdle` fast-path，也没有把更高层 `AppLifecycle::quit()` / runtime shutdown 调用链一并重写；它只删除了私有 helper 中已经失效的备用分支，并把“pipeline 必须先退场”固化成断言。

- 2026-07-16：Forward / Deferred 的 shadow resource rebuild 也开始从“refresh 内层再整机 idle 一次”收紧回现有 frame-boundary contract。代码时序复盘确认，两条 pipeline 的 `applyPendingResourceRefreshes()` 都是在 `RenderRuntime::prepareFrame()` 成功之后、也就是 `_render->begin()` 已经完成当前帧 fence wait / DDQ flush / acquire 之后执行；而且 shadow refresh 发生在当前帧第一次真正录制 shadow pass 之前。此前两条路径即便已经处在这套标准帧边界之后，只要命中 `ShadowResources` refresh 且旧 shadow image 存在，仍会再 `_render->waitIdle()` 一次，然后才 destroy/init shadow resources。现在这层内置全局等待已删除，refresh 继续只负责旧 shadow owner 退休与新资源创建，等待责任回收到已有的 frame-boundary sync。
- 直接收益：这一步没有把 shadow rebuild 宣称为“彻底不需要同步”，但它确实让 Forward/Deferred 和上一批 pipeline switch 一样，更一致地依赖当前单 flight Vulkan 后端已经存在的帧边界 fence contract。后续如果继续看 shadow resolution change、disable/enable、pipeline switch 或 registry replacement 的交界，观察到的行为会更接近真实 owner 替换语义，而不是先被 refresh 内层的 `waitIdle()` 全部抹平。
- 当前停止线：这次结论同样依赖当前 Vulkan `flightFrameSize = 1` 与 refresh 发生在 `begin()` 之后的时序，没有把未来多 frames-in-flight、OpenGL 路径或更细的 resource-retirement contract 一并提前假定为已成立；它只删除了当前帧边界已经覆盖掉的那层 shadow refresh 内置全局等待。

- 2026-07-16：Phase 0 里“可重复行为基线”这条现在终于不再只是口头成立了。除了已经落档的 Deferred / Forward 默认截图基线与 postprocess smoke，本轮继续把代码里早就支持、但计划目录里还没有版本化入口的两条行为型 smoke 补成了固定 config：`.agent/plan/render-resource-and-graph-refactor/viewport-resize-smoke.automation.json` 和 `pipeline-switch-smoke.automation.json`。前者固定在 frame 50 触发 `960x540` viewport resize，并在 frame 180 抓 editor screenshot；后者固定在 frame 50 从默认 Deferred 切到 Forward，并在 frame 180 抓切换后截图。两条命令都已在当前工作树实际跑通，分别产出 `/tmp/viewport-resize-smoke-2026-07-16.png`（sha1 `bd77997218ebe8ba88e97b80903626b1b6e57381`）和 `/tmp/pipeline-switch-smoke-2026-07-16.png`（sha1 `24578fd8b15569e66fbcfb53be888a3cf04b7bbd`）；截图里前者保持 Deferred 正常出图，后者确认 runtime 已稳定切到 Forward 且 PBR 球体仍保有环境反射。这样后续再动 registry replacement、pipeline switch、viewport attachment rebuild、postprocess output 或 editor screenshot 链时，不需要每次重新手拼 CLI 覆盖，也不用再把“代码支持但没有版本化入口”误当作计划已落地。
- 直接收益：`todo.md` 里 Phase 0 的“后续资源或 graph 迁移有可重复行为基线”现在终于有了更完整的当前态支撑；底部持续审查项里与当前现实已经对齐的 `make b t=HelloMaterial`、Forward/Deferred 固定帧运行、viewport resize、bloom/postprocess/ACES 与 Forward/Deferred switch 也可以按既有证据勾正，不再让计划文件继续暗示这些入口还不存在或尚未打通。
- 当前停止线：这一步仍然没有把 `shadow`、`SSAO` 单独开关、`screenshot/readback` 和 `shutdown 无 validation/lifetime error` 一并宣称完成。当前补齐的是高复用、与当前主线最相关的版本化 smoke 入口和计划现实对齐；更高层的 shutdown/resource-lifetime 证明仍需要后续继续做针对性验证，而不是借这次文档收口顺手过度勾选。
...
- 2026-07-16：runtime / simulation -> editor 的 play-mode 退出边界上，也继续收掉了一层“其实并不在 play mode”时的空转全局等待。此前 `AppLifecycle::stopRuntime()` / `stopSimulation()` 只要当前 `appState` 分别是 Runtime / Simulation，就会先无条件 `render->waitIdle()`，随后再调用 `SceneManager::exitPlayMode()`；但 `SceneManager::exitPlayMode()` 在 `_playScene` 不存在时本身就是 no-op，只会把 active scene 指回 editor scene 并返回 false。这意味着如果上层状态与 scene manager 的 play-mode 持有状态已经不同步，或某条恢复路径已经提前退出了 play mode，这两条 stop 路径仍会先让整台 device 全局 idle，尽管并没有旧 play-scene owner 需要 teardown。现在两条路径都只在 `SceneManager::isInPlayMode()` 为真时才等待；真正涉及 play-scene 退出的 runtime/simulation stop 仍保留等待，而“不在 play mode”的空路径则不再先做一次全局 idle。
- 直接收益：这一步继续压的是 lifecycle / mode-transition 边界，而不是回到 execute-path 或 facade 清理。play-mode stop 的同步语义现在进一步贴近真实 owner contract：只有当前确实持有 play-scene、即将 exitPlayMode 时才 quiesce GPU；空 stop 路径不再被一刀切地按“先全局等一次”处理。
- 当前停止线：这次没有删除真实 play-mode exit 边界本身的 `waitIdle()`，也没有声称 scene replacement、pipeline switch 或 runtime shutdown 已具备更细的 submit/fence 退休语义；只是在“当前不在 play mode”这个可证实的子场景下去掉空转等待。后续若继续推进，仍应围绕真正持有旧 play-scene / render owner 的 mode transition 与 shutdown 边界逐条审。
...
- 2026-07-16：Deferred pipeline frame preparation 里残留的默认 skybox primitive 查询也进一步前移了。此前 `DeferredRenderPipeline::updateStageFrameInputs()` 在给 `ViewportOverlayStage` 组装 skybox frame input 时，只要场景里存在可渲染 skybox，就会临时通过 `PrimitiveMeshCache::get().getMesh(EPrimitiveGeometry::Cube)` 现取默认 cube mesh，再按场景里是否有显式 skybox mesh 去覆盖；这让 Deferred 每帧 stage input preparation 虽然已经显式化到 pipeline，但默认 skybox 路径仍在 frame preparation 里摸全局 primitive cache。现在 pipeline 在 `initPipelineState()` 时就缓存默认 skybox cube，后续 frame input 只消费缓存结果或场景显式 mesh，不再在每帧 preparation 期查询 primitive singleton。
- 直接收益：这一步继续压的是 Deferred 主链“frame preparation 只组装显式输入，不顺手回头查全局 primitive 资源”这条现实 contract，而且它紧接着前几批 execute cleanup，把同一条 skybox/overlay 主线从 graph pass 内延伸到 pipeline frame setup。这样 `ViewportOverlayStage` 的 skybox 输入来源更清楚：要么来自场景已解析 mesh，要么来自 pipeline 初始化时锁定的默认 cube，而不是每帧临时回查 global cache。
- 当前停止线：这次只收了 Deferred 默认 skybox mesh 这一条 frame-preparation primitive query，并没有把 `waitIdle()` 边界问题强行和它打成一批，也没有把 Forward 的同类 skybox default mesh 查询一起扫掉；下一优先级仍应回到更硬的 runtime/shutdown/registry boundary，而不是继续把所有 setup-time global query 机械清空。
- 2026-07-16：Deferred Viewport Overlay 的 execute/prepare 路径里，对 debug render service 的“按需回调查询”也进一步收成了显式 stage state。此前 `ViewportOverlayStage` 虽然已经通过 `Services` 注入 debug render system，但在 `refreshPipelineFormats()`、`prepare()`、`drawOverlay()` 和 GUI 路径里仍会反复调用 `_getDebugRenderSystem()` 取回引用；这不再是直接摸 singleton，但 deferred overlay execute path 依旧没有完全只消费 init/prepare 后的显式成员。现在 stage 在 `init()` 时就缓存 `DebugRenderSystem*`，后续只消费缓存指针，不再在 graph 常驻 stage 的执行期反复走 service 回调。
- 直接收益：这一步继续压的是 `todo.md` 里 “graph execute 是否还有全局查询” 的更严格现实 contract，而且它与前两批 Deferred execute cleanup 是同一条线：让 overlay stage 的执行期尽量只依赖已经同步好的成员状态。到这里，Deferred overlay 这条 graph-backed 路径在 primitive mesh 和 debug service 两侧都更接近纯 state consumer。
- 当前停止线：这次只把 `ViewportOverlayStage` 内部的 debug render service 访问前移成 cached pointer，并没有把 `RenderRuntime -> getDebugRenderSystem()` 这类更外层 service 提供方式整体改成新的 owner/interface，也没有把 Forward overlay 的同类依赖一起打包；后续若继续推进，应优先围绕更高价值的 runtime/shutdown/registry boundary，而不是继续把所有 service accessor 机械扫平。
- 2026-07-16：Deferred Viewport Overlay 这条 graph execute 路径里残留的 execute-time primitive singleton 查询也继续前移了。此前 `ViewportOverlayStage::drawOverlay()` 在真正录制方向 gizmo 的 cone / cylinder draw 前，仍会临时通过 `PrimitiveMeshCache::get()` 现取 mesh；这让 deferred 主图里的 overlay graph pass 虽然已经吃 frame snapshot 和 stage state，但一到方向 gizmo 录制时仍会在 graph pass 期回头摸全局 primitive cache。现在 `ViewportOverlayStage::init()` 会预先缓存 direction cone / cylinder，`drawOverlay()` 只消费缓存结果，不再在 graph 录制期查询 primitive singleton。
- 直接收益：这一步继续压的是 `todo.md` 里 “graph execute 是否还有全局查询” 这条现实 contract，而且它和上一批 Deferred Light 一样，直接作用在 Deferred 主图常驻 stage 的 execute path。Deferred overlay 现在更接近纯 state consumer：graph execute 时只依赖准备好的 frame snapshot、debug system 和 cached primitive mesh，不再边录制边查全局 primitive 资源。
- 当前停止线：这次只收了 Deferred `ViewportOverlayStage` 里 direction gizmo 这条 execute-time primitive query，没有把 Forward overlay 的同类 `PrimitiveMeshCache::get()`、也没有把更高层的 debug system service 访问一起扩成同一批；后续仍要按主线价值逐条审，避免把 Deferred/Forward/global service 一锅端。
- 2026-07-16：Deferred Light 这条 graph execute 路径里残留的 execute-time primitive singleton 查询也已前移。此前 `LightStage::execute()` 虽然已经只消费当前帧 descriptor set 和 GBuffer snapshot，但在真正录制 fullscreen lighting draw 时，仍会通过 `PrimitiveMeshCache::get().getMesh(EPrimitiveGeometry::Quad)` 现取一次全屏 quad；这让 deferred 主图 execute callback 虽然表面不再自己 `::get()`，其下游 stage execute 仍在 graph pass 录制期摸全局 primitive cache。现在 `LightStage::init()` 会预先缓存这张 fullscreen quad，`execute()` 只消费缓存结果，不再在 graph 录制期查询 primitive singleton。
- 直接收益：这一步继续压的是 `todo.md` 里 “graph execute 是否还有全局查询” 的现实 contract，而且比 presentation 那条更接近 Deferred 主链。Deferred Light pass 现在更像一个纯粹的已同步 state consumer：graph execute 时只绑定 pipeline / descriptor / cached mesh，然后录制 draw，不再顺手回头查全局 primitive 资源。
- 当前停止线：这次只收了 Deferred Light 这条 execute-time primitive query，并没有把 `ViewportOverlayStage`、Forward overlay 或其他仍在 stage execute 内部查 `PrimitiveMeshCache` 的路径一起打包进来；后续仍需按主线价值逐条审，不把所有 primitive 查询一锅端进同一批。
...
- 2026-07-16：RenderGraph 里 imported buffer 这条 contract 现在也开始补 final-state 收口，而不再只支持 initial-state seed。此前 imported texture 在 graph 结束后已经会回到声明的 `finalLayout`，但 imported buffer 仍只有 `initialState`；像 screenshot readback 这种 graph 内 `TransferDst`、graph 外立即 map/read 的路径，结束后并没有一条显式的 `TransferWrite -> HostRead` barrier。现在 `RGImportedBufferDesc` 新增可选 `finalState`，executor 在 pass 执行完后会对 imported buffer 做同样的末端状态收口；同时 screenshot readback buffer 已显式声明 `HostRead` final state，并新增 `RenderGraphCoreTest.ExecutorRestoresImportedBufferFinalStateAfterTransferPass` 锁住行为。
- 直接收益：这一步继续压的是 plan 里一直挂着的 imported buffer state / shutdown consistency，而不是再去做一刀 owner 字段整理。graph-backed readback、copy output 和后续任何“graph 内写 buffer、graph 外继续消费”的路径，现在终于和 imported texture 一样，有了清晰的末端 state contract。
- 当前停止线：这次只补了 imported buffer final-state 的 graph core contract 和 screenshot readback 这条高价值 consumer；它还没有把所有 imported buffer callsite 都逐条声明 final state，也没有把 runtime/shutdown 上更高层的 owner/replacement 边界问题一并算作已完成。
