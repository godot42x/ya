...
5. **GPU UBO 结构体必须使用 ya-shader 从 `.slang` 生成的 C++ 类型（`slang_types::` 命名空间）作为唯一数据源。C++ 侧不得自行定义与 shader uniform 对应的结构体。** 例如：`PBRMaterial::ParamUBO` 应使用 `slang_types::DeferredRender::GBufferPass_PBR::PBRParamsData`，而不是手写 `struct ParamUBO { ... };`。
