...
- `resource-system`：涉及 TextureSlot resolve、AssetManager、descriptor 刷新时一起看
- `render-arch`：涉及 render pass / pipeline 如何消费 runtime material 时一起看
- `cpp-style`：需要保持单一事实源、最小抽象和清晰状态流时一起看
- `debug-review`：排查材质串扰、白纹理、局部回归时一起看
- `ya-build`：改到 shader 生成头、测试或编译错误时一起看
...
- 只负责已有组件的运行时资源解析
- 不创建 scene topology
- 当前职责包括：Mesh / PhongMaterial / PBRMaterial / UnlitMaterial / UI / Billboard / Skybox / EnvironmentLighting resolve
...
5. **GPU UBO 结构体必须使用 ya-shader 从 `.slang` 生成的 C++ 类型（`slang_types::` 命名空间）作为唯一数据源。C++ 侧不得自行定义与 shader uniform 对应的结构体。** 例如：`PBRMaterial::ParamUBO` 应使用 `slang_types::DeferredRender::Unified_GBufferPass_PBR::PBRParamsData`，而不是手写 `struct ParamUBO { ... };`。
